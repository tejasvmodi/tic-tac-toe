...
def restartgame(): 
    ...
    titleLabel1=Label(fram1,text="Tic Tac Toe" , font=("Arial",30), bg="orange", width=15)
    titleLabel1.grid(row=0,column=0)
...
def play(event):
    ...
    if button['text']==" ":
        if turn == "X":
            board[clicked]=turn
            if checkforwin(turn):
                winningLabel=Label(fram1,text=f"{turn} wins the game ",bg="orange",font=("Arial",26),width=16)
        ...
        if checkfordraw():
            drawLabel=Label(fram1,text="game draw", bg="orange",font=("Arial",26),width=16)
